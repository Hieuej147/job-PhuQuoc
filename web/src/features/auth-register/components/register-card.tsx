    ...
    <div className="animate-in rounded-3xl border border-primary bg-primary-foreground p-8 shadow-xl delay-100">
      <div className="flex flex-col gap-5">
        ...
        <p className="text-center text-sm text-muted-foreground">
          ...
          <Link
            href="/auth/login"
            className="ml-1 font-bold text-primary transition-opacity hover:opacity-70"
          >
            Đăng nhập →
          </Link>
        </p>
...
function RegisterHeader() {
  return (
    <div className="flex flex-col gap-1">
      <h1 className="mb-1 text-2xl font-bold text-secondary-foreground">
        Tạo tài khoản mới 🚀
      </h1>
...
function RoleStep({ form }: { form: ReturnType<typeof useRegisterForm> }) {
  const roles = [
    {
      value: "CANDIDATE" as RegisterRole,
      icon: User,
      label: "Ứng viên",
      desc: "Tìm việc làm",
    },
    {
      value: "EMPLOYER" as RegisterRole,
      icon: Building2,
  ...
  return (
    <div className="flex flex-col gap-5">
      <GoogleButton
        onClick={form.handleGoogleRegister}
            ...
            <button
              ...
              className={cn(
                ...
                form.role === role.value
                  ...
                  : "border-border bg-background hover:border-[#0E7490]/50",
              )}
            >
              <div
                className={cn(
                  "flex size-12 items-center justify-center rounded-2xl",
                  form.role === role.value ? "bg-[#0E7490]/15" : "bg-muted",
                )}
              >
                <role.icon
                  className={cn(
      ...
      <Button
        onClick={form.goNext}
        className="h-11 w-full rounded-xl bg-primary font-bold shadow-md hover:bg-[#005a71]"
      >
        Tiếp tục →
      </Button>
    </div>
  ...
  return (
    <div className="flex flex-col gap-4">
            ...
            {[0, 1, 2, 3].map((index) => (
              ...
              >
                <div
                  ...
                  style={{ width: index < strength ? "100%" : "0%" }}
                />
              </div>
            ))}
          </div>
      ...
      <PasswordField
        ...
        setShow={form.setShowConfirm}
        error={form.errors.confirm}
        onChange={(value) => {
          form.setConfirmPass(value);
          form.setFieldError("confirm", "");
        }}
      />
      <TermsCheckbox
        checked={form.terms}
        onChange={() => form.setTerms(!form.terms)}
      />
      <StepButtons
        onBack={form.goBack}
        onNext={form.handleRegister}
        nextLabel={form.loading ? "Đang tạo..." : "Tạo tài khoản 🎉"}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-sm font-semibold text-muted-foreground">
        ...
        {props.hint && (
          <span className="text-xs font-normal">{props.hint}</span>
        )}
      </label>
      <div className="relative">
        ...
        <Input
          ...
          className="h-11 rounded-xl bg-[#f7f9ff] pl-10"
        />
      </div>
    ...
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="h-11 rounded-xl bg-[#f7f9ff] pl-10 pr-10"
        />
        <button
          type="button"
  ...
  return (
    <label className="group flex cursor-pointer items-start gap-3">
      ...
      <span className="text-xs leading-relaxed text-muted-foreground">
        Tôi đồng ý với{" "}
        <span className="font-semibold text-primary hover:opacity-70">
          Điều khoản sử dụng
        </span>{" "}
        và{" "}
        <span className="font-semibold text-primary hover:opacity-70">
  ...
  return (
    <div className="flex gap-3">
      ...
      <Button
        ...
        className={cn(
          "h-11 flex-1 rounded-xl font-bold shadow-md",
          props.submit
            ? "bg-[#F59E0B] text-white hover:bg-[#D97706]"
            : "bg-[#0E7490] text-secondary-foreground hover:bg-[#005a71]",
        )}
      >
        {props.nextLabel}
      </Button>
