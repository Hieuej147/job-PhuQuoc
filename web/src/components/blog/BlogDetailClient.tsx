...
export default function BlogDetailClient({
    ...
    const scrollToHeading = (index: number) => {
        ...
        if (headingElements[index]) {
            const y = headingElements[index].getBoundingClientRect().top + window.pageYOffset - 100;
    ...
    const authorInitials = authorName ? authorName.slice(0, 2).toUpperCase() : "BB";
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
                            <div className="lg:col-span-3">
                                ...
                                <span className="inline-flex items-center gap-1.5 bg-white/15 backdrop-blur-sm text-white text-xs font-bold px-3 py-1.5 rounded-full border border-white/20 mb-4">
                                    <span className="material-symbols-outlined text-[14px]">hotel</span> {categoryName}
                                </span>
                                ...
                                <div className="flex flex-wrap items-center gap-5 text-white/65 text-xs">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-white/85 font-semibold">{authorName}</span>
                                    </div>
                                    <span className="flex items-center gap-1">
                                        <span className="material-symbols-outlined text-[14px]">calendar_today</span> {formattedDate}
                                    </span>
                                    <span className="flex items-center gap-1">
                                        <span className="material-symbols-outlined text-[14px]">schedule</span> 6 phút đọc
                                    </span>
                                    <span className="flex items-center gap-1">
                                        <span className="material-symbols-outlined text-[14px]">visibility</span> {formatViews(blog.views)} lượt xem
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="lg:col-span-3">
                        <article id="article-content" className="fade-up stagger-1">
                            ...
                            <div className="mt-6 flex flex-wrap items-center gap-3">
                                <button
                                    onClick={handleLike}
                                    className={`flex items-center gap-1.5 px-[18px] py-2 border rounded-full text-xs font-semibold cursor-pointer transition-all duration-200 ${liked
                                ...
                                <button className="flex items-center gap-1.5 px-[18px] py-2 border border-[#e0f5fb] dark:border-[#1a3d5c] rounded-full text-xs font-semibold cursor-pointer bg-white dark:bg-[#0d2137] text-slate-500 dark:text-[#94a3b8] hover:border-[#0e7490] hover:text-[#0e7490] transition-colors duration-200">
                                    <span className="material-symbols-outlined text-[18px]">bookmark</span> Lưu bài
                                </button>
                            </div>
                            ...
                            <div className="mt-8 bg-white dark:bg-[#0d2137] border border-[#e0f5fb] dark:border-[#1a3d5c] rounded-2xl p-5 flex items-start gap-4 shadow-sm">
                                ...
                                <div>
                                    <p className="font-bold text-sm text-[#001e30] dark:text-[#e0f2fe]">{authorName}</p>
                                    <p className="text-xs text-slate-500 dark:text-[#94a3b8] mb-2">Chuyên gia nhân sự ngành khách sạn & du lịch Phú Quốc</p>
                                    <p className="text-sm text-slate-600 dark:text-[#cbd5e1]">Đội ngũ biên tập PQJobs với hơn 10 năm kinh nghiệm trong lĩnh vực tuyển dụng và đào tạo nhân lực du lịch tại Phú Quốc.</p>
                                </div>
                            </div>
                        </article>
                                ...
                                <nav className="space-y-0.5">
                                    {headingsList.map((heading, i) => (
                                        <button
                                            ...
                                            className={`w-full text-left toc-link block text-xs py-1.5 px-2 rounded-lg transition-colors leading-relaxed ${activeIndex === i
